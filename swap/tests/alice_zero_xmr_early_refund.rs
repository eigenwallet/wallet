...
use harness::{setup_test_with_alice_starting_balances, StartingBalances};
...
async fn alice_zero_xmr_refunds_bitcoin() {
    setup_test_with_alice_starting_balances(
        StartingBalances::new(bitcoin::Amount::ZERO, swap::monero::Amount::ZERO, None),
        FastCancelConfig,
        |mut ctx| async move {
            ...
            assert!(matches!(alice_state, AliceState::EarlyRefunded { .. }));
            assert!(matches!(bob_state, BobState::EarlyRefunded { .. }));
