...
async fn test_monero_wrapper_with_harness() {
    ...
    let (address, wallet_seed) = create_monero_wrapper_wallet(&wallet_path).await;
    ...
    let tc = Cli::default();
    let (monero, monerod_container, _wallet_containers) = 
        Monero::new(&tc, vec![]).await.expect("Failed to create Monero containers");

    let daemon_address = get_daemon_address(&monerod_container);

    // Initialize miner
    info!("Initializing miner wallet");
    monero.init_miner().await.expect("Failed to initialize miner");
    ...
    init_to_wallet_address(&monero, &address, FUND_AMOUNT).await
    ...
    assert!(wallet_balance > 0, "Wallet balance should be greater than 0");
    
    info!("Test passed! Wallet successfully received and detected funds");
}

async fn create_monero_wrapper_wallet(wallet_path: &str) -> (String, String) {
    // Get wallet manager
    ...
    let wallet = wallet_manager.recover_wallet(
        wallet_path,
        PASSWORD,
        seed,
    ...
    let address = wallet.address(0, 0);
    
    (address, seed.to_string())
}

async fn init_to_wallet_address(monero: &Monero, address: &str, amount: u64) -> anyhow::Result<()> {
    info!("Funding address {} with {} piconero", address, amount);
    
    // Generate some blocks to ensure miner has funds
    monero.fund_address(address, amount).await?;
...
fn get_temp_wallet_path() -> String {
    ...
    let uuid = Uuid::new_v4(); // This is the correct method to generate a random UUID
...
fn get_daemon_address(monerod_container: &Container<'_, Monerod>) -> String {
    let local_daemon_rpc_port = monerod_container.ports().map_to_host_port_ipv4(monero_harness::image::RPC_PORT);
    let local_daemon_rpc_port = local_daemon_rpc_port.expect("monerod should have a mapping to the host for the default RPC port");
    
    format!("localhost:{}", local_daemon_rpc_port)
}

async fn connect_and_check_balance(seed: String, daemon_address: String) -> u64 {
    // Get wallet manager
    let wallet_manager_mutex = WalletManager::get();
    let mut wallet_manager = wallet_manager_mutex
        .lock()
    ...
    // Check connection
    let connected = wallet_manager.connected();
    ...
    let mut wallet = wallet_manager.recover_wallet(
        ...
        PASSWORD,
        &seed,
        NetworkType::Mainnet, // Regtest uses Mainnet addresses
    ...
    // Initialize wallet
    wallet.init(&daemon_address, false);
    
    tracing::info!("Starting testing of wallet with address: {}", wallet.address(0, 0));
