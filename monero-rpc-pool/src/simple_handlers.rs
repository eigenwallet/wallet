use axum::{extract::State, response::Json, extract::Path, http::StatusCode};
...
async fn make_single_request(
    ...
    node_url: String,
    payload: &Value,
) -> Result<(Value, String, f64), HandlerError> {
...
async fn handle_rpc_request(state: &AppState, payload: &Value) -> Result<Value, HandlerError> {
...
async fn handle_http_request(state: &AppState, endpoint: &str) -> Result<Value, HandlerError> {
    let (node1, node2) = get_two_nodes(state).await?;
    let client = create_client()?;
    
    debug!("Racing HTTP requests to {}: {} and {}", endpoint, node1, node2);
...
pub async fn simple_rpc_handler(
    ...
    debug!("Received JSON-RPC request: {}", serde_json::to_string_pretty(&payload).unwrap_or_else(|_| "Invalid JSON".to_string()));
    
    match handle_rpc_request(&state, &payload).await {
        Ok(response) => {
            debug!("Sending JSON-RPC response: {}", serde_json::to_string_pretty(&response).unwrap_or_else(|_| "Invalid JSON".to_string()));
            Json(response)
        }
        Err(error) => {
            let error_response = error.to_json_response(payload.get("id"));
            debug!("Sending JSON-RPC error response: {}", serde_json::to_string_pretty(&error_response).unwrap_or_else(|_| "Invalid JSON".to_string()));
            Json(error_response)
        }
    }
}
...
pub async fn simple_http_handler(
    ...
    match handle_http_request(&state, &format!("/{}", endpoint)).await {
        Ok(response) => {
            debug!("Sending HTTP response: {}", serde_json::to_string_pretty(&response).unwrap_or_else(|_| "Invalid JSON".to_string()));
            Ok(Json(response))
        }
        Err(error) => {
            let error_response = error.to_json_response(None);
            debug!("Sending HTTP error response: {}", serde_json::to_string_pretty(&error_response).unwrap_or_else(|_| "Invalid JSON".to_string()));
